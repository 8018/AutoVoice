...
/** Shared ASR, business LLM and navigation assembly for both speech build variants. */
@Configuration
public class BusinessBackendConfig {
    ...
    @Bean
    public AliyunTokenClient aliyunTokenClient(OkHttpClient client,
                                                AppConfig.AutovoiceProperties props) {
        return new AliyunTokenClient(
                client, props.secrets().aliyunAk(), props.secrets().aliyunSk());
    }

    @Bean
    public AsrProvider businessAsrProvider(OkHttpClient client, AliyunTokenClient tokenClient,
                                           Clock clock, AppConfig.AutovoiceProperties props) {
    ...
    public LlmProvider businessLlmProvider(OkHttpClient client,
        ...
        ToolProvider businessTools = () -> {
            List<FunctionTool> tools = new ArrayList<>(DeepSeekLlmProvider.defaultTools());
            tools.addAll(registry.enabledToolSpecs());
            return tools;
        };
