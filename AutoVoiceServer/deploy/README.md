...
- **支持窗口**：动作身份签发时带截止时刻（默认 24h），随 `reply.actionExpiresAtMs` 下发。
  超过窗口的请求由客户端**明确拒绝**（不执行、不记账）——旧请求重放不会重复执行副作用。
- **审计独立**：动作账本库内 `action_audit` 表 append-only 记录签发等事件，**独立于动作最新状态**
  （不能只保留最新状态）；审计保留期与幂等记录保留期是两件事，前者的清理不影响后者对窗口内
  请求的判定。
- **写入失败的处理（约束：审计失败与动作失败分开）**：
  - 执行前账本/审计写入失败 → 服务端**拒绝下发动作身份**（降级为不带 actionId 的回复，
    客户端因此不执行），播报文本照常；
  - 执行后结果记录失败 → 客户端进入结果待核实（`RESULT_UNKNOWN`），不重试、不否认；
  - 普通诊断队列拥塞可降级，不影响语音识别与只读查询。
