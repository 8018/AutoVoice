...
端侧离线命令识别支持“退出”“退出对话”“结束对话”“退出当前对话”“结束当前对话”。命中后
产出 `conversation/exit_dialogue`，与车窗硬规则命令一样立即进入端侧仲裁 FIFO；先入队即胜出，
不等待云端返回。业务处理器收到胜出语义后停止当前播报、清除待选择的导航任务、退出闲聊模式，
并把本地会话状态机复位到待唤醒态。仲裁器不取消端侧或云端生产者，迟到结果由复位后的会话准入
检查拦截。该意图只退出语音对话，不关闭 Android 应用。
