...
- 设置区默认“在线”；debug APK 连接 dev、release APK 连接生产，设置区不提供环境切换；
  在线模式下可开「模拟弱网（云端延迟 3s）」；
...
| 4.5 | 降噪流连续性 | 连续输入 N 个 1024B 采集块，跨块重组 480-sample RNNoise 帧；停录后输出总字节数必须等于 N×1024，无周期性断音 |
