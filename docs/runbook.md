...
| 环境变量 | 用途 | 来源 |
|---|---|---|
| `XFYUN_APPID` | 讯飞语义 API（AIUI）AppID——云端传统 NLU | 讯飞开放平台 → AIUI 控制台（应用详情） |
| `XFYUN_API_KEY` | 讯飞语义 API Key——云端传统 NLU | 同上 |
| `DEEPSEEK_API_KEY` | DeepSeek LLM——云端 LLM 兜底（剧本 3） | DeepSeek 开放平台 |
...
- **期望**：
  ...
  2. 云端决策日志：`nlu_first`（传统 NLU 在 1500ms 宽限内到达并命中；
     若 LLM 先到则允许为 `llm_first_wait_nlu_arrived`）；
  3. 模拟车控面板**温度 = 24**；
  4. **阿里云 TTS 播报**（云端回复音频播放）。
- **验证点**：云端仲裁传统分支 + 云端传统语音链路（讯飞 ASR → 讯飞语义 NLU →
  阿里云 TTS）。

### 剧本 3：云端 LLM 兜底

- **前置**：同上（在线 + `demo-full`）。
- **操作**：按住录音说 **"明天上海天气怎么样"**（或 **"讲个笑话"**——规则 NLU
  词表外的任意闲聊句均可）。
- **期望**：
  1. 云端决策日志：`nlu_rejected_use_llm`（传统 NLU 拒识/置信度低 → LLM 兜底；
     若 LLM 先到等传统则允许 `llm_first_wait_timeout`）；
  2. **TTS 播报 LLM 回答文本**（DeepSeek 生成的内容，经阿里云 TTS 或文本兜底播报）。
- **验证点**：云端仲裁 LLM 兜底 + 云端大模型链路。
- **注**：演示 1500ms「LLM 先于传统到达」窗口时序不可控，不列为必验项
  （已由仲裁收敛单测覆盖）。
...
### 5.3 TTS 静默降级（kind=text）

云端回复分 `audio` / `text` / `action` 三种 kind：

- `audio`（阿里云 TTS 合成的 WAV）→ 音频播放；
- `text`（如 LLM 兜底回复、safety 兜底话术）→ **系统 TTS 播报**（不静默）；
- 阿里云 TTS 不可用时服务端仲裁层以文本兜底收敛（`safety_timeout` 兜底话术），
  客户端侧不受影响。
