...
1. **断网本地兜底**：断网 → "打开空调" → 端侧仲裁离线兜底 → 讯飞离线链路
2. **本地覆盖**：在线 → "打开空调" → 端侧仲裁本地覆盖 → 讯飞离线链路
3. **闲聊走 LLM**：在线 → "明天上海天气怎么样" → 端侧上云 → 云端仲裁非车控 → LLM 对话
4. **车控走传统 NLU**：在线 → "空调调到24度" → 端侧上云 → 云端仲裁车控白名单 → 传统 NLU → mock 执行 + TTS 播报
