...
- 待确认点 1 通过:actionId 放 `reply.payload` 顶层。
- 待确认点 2 通过:首期不开放服务端账本的客户端查询接口。
- 实施顺序与补齐要求(先补齐再 D07a→D07b):
  1. **持久化动作身份**:actionId 的签发记录必须落盘(服务端账本),不依赖内存;
  2. **客户端最终准入**:执行前由客户端执行入口做最终校验(当前轮、actionId 已签发且未执行、参数/状态合法);
  3. **原子执行抢占**:并发/重复触达时以账本原子 claim 保证只执行一次;
  4. **崩溃未知态恢复**:启动时把 EXECUTING 记录收敛为 RESULT_UNKNOWN,不自动重试。
- 真实车控继续关闭。
