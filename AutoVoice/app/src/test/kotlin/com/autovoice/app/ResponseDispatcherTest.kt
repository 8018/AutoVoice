...
class ResponseDispatcherTest {
    ...
    @Test
    fun `cloud action reply speakText is shown in reply window`() {
        val texts = mutableListOf<String>()
        dispatcher { texts += it }
            .dispatchCloud(
                "t1",
                ActionReply(
                    intent = Intent("1.0", "navigation", "start_navigation", emptyMap(), 0.9, "protocol"),
                    speakText = "开始导航",
                ),
            )
        assertEquals(listOf("开始导航"), texts)
    }
